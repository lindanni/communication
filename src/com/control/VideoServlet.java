		...
		try {
			...
			while (iter.hasNext()) { //循环处理所有上传的文件
			    ...
			    if (item.isFormField()) {
			    	 ...
			    	 //文件的上传目录
			    	 String filedir=webroot+"uploadvideo"+"/"+now;
			    	 ...
			    	 if(suffix.equalsIgnoreCase(".mp3") || suffix.equalsIgnoreCase(".mp4") || suffix.equalsIgnoreCase(".wmv") || suffix.equalsIgnoreCase(".mkv") || suffix.equalsIgnoreCase(".mov")){
			    		 //用当前时间来命名上传文件的名字
				    	 String lasttime=new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());;
				    	 
				    	 //文件在数据库中保存路径
				    	 String filepath="uploadvideo"+"/"+now+"/"+lasttime+suffix;
						...
						request.setAttribute("sizeInBytes", sizeInBytes);
				    	 ...
				    	 System.out.print("路径："+filepath);
				    	 item.write(uploadedFile); 
				    	 
				    	 message="<div style='color:red'>上传成功</div>";
				    	 
			    	 }else{
			    		 message="<div style='color:red'>视频类型非法</div>";
			    	 }	  	
			    	
			    }
			}
			request.setAttribute("message", message);
			request.getRequestDispatcher("upvideo.jsp").forward(request, response);
