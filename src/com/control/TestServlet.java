		...
		try {
			...
			while (iter.hasNext()) { //循环处理所有上传的文件
			    ...
			    if (item.isFormField()) {
			    	 ...
			    	 //文件的上传目录
			    	 String filedir=webroot+"uploadtest"+"/"+now;
			    	 ...
			    	 if(suffix.equalsIgnoreCase(".txt") || suffix.equalsIgnoreCase(".doc") || suffix.equalsIgnoreCase(".docx")){
			    		 //用当前时间来命名上传文件的名字
				    	 String lasttime=new SimpleDateFormat("yyyyMMddhhmmss").format(new Date());;
				    	 
				    	 //文件在数据库中保存路径
				    	 String filepath="uploadtest"+"/"+now+"/"+lasttime+suffix;
			    		 ...
			    		 message="<div style='color:red'>试题类型非法</div>";
			    	 }	  	
			...
			request.getRequestDispatcher("uptest.jsp").forward(request, response);
